...
void dataserver::message_process_getdata(Socket sk)
{
    ...
    char buff[SEG_SIZE] = {0}; // buffer for segement data

    // frange
    string file = frange.file();
    int fileID = frange.fileid();
    int segno = frange.segno();
    int offset = frange.offset();
    int length = frange.length();

    string segmentFileName = file + to_string(fileID) + "_" + to_string(segno);


    // client wants to read
    if (msg.msgtype() == msgcontainer::FILEREAD) {
        readLocalFile(buff, segmentFileName, offset, length);
...
void dataserver::readLocalFile(char *buff, string segmentFileName, int offset, int length)
{
